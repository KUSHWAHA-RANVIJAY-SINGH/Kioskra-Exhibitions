...
import { MapPin, Layers, ArrowUpRight, SlidersHorizontal, Maximize2 } from "lucide-react";
...
export default function ProjectsCatalogClient({ projects }: ProjectsCatalogClientProps) {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {/* Category Tabs */}
      <div className="flex flex-wrap items-center gap-2.5 pt-0 pb-4 border-b border-brand-softStone">
          ...
          {filteredProjects.map((project) => (
            ...
            >
              <Link
                ...
                className="group flex flex-col rounded-3xl overflow-hidden bg-white border border-brand-softStone hover:border-brand-electricBlue/40 transition-all duration-300 shadow-sm hover:shadow-xl h-full no-underline text-inherit"
              >
                {/* Image Section */}
                ...
                <div className="p-6 flex flex-col justify-between flex-grow gap-5">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-[11px] font-bold text-brand-electricBlue uppercase tracking-wider">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{project.venue ? project.venue.split(",")[0] : project.location}</span>
                      </div>
                      <span className="bg-brand-softStone/40 px-2 py-0.5 rounded text-[10px]">
                        {project.areaSize || "Custom Stall"}
                      </span>
                    </div>
                  ...
                  <div className="pt-4 border-t border-brand-softStone/60 flex items-center justify-between text-xs font-bold text-brand-charcoal/80">
                    ...
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-brand-electricBlue/10 text-brand-electricBlue font-extrabold group-hover:bg-brand-electricBlue group-hover:text-white transition-all duration-300">
                      <span>Explore</span>
