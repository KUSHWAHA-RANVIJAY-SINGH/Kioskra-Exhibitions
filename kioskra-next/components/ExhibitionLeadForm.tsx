...
interface ExhibitionLeadFormProps {
  exhibitionTitle: string;
  exhibitionVenue: string;
  exhibitionLocation: string;
}

interface ExhibitionLeadFormInputs {
  clientName: string;
  company: string;
  email: string;
  phone: string;
...
export default function ExhibitionLeadForm({
  exhibitionTitle,
  exhibitionVenue,
  exhibitionLocation,
}: ExhibitionLeadFormProps) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<ExhibitionLeadFormInputs>({
    mode: "onBlur",
    defaultValues: {
      clientName: "",
      company: "",
      email: "",
      phone: "",
      eventCity: exhibitionLocation, // Pre-fill with exhibition city/location
      stallRequirement: "",
      message: "",
    },
  });
  ...
  const onSubmit = async (data: ExhibitionLeadFormInputs) => {
    ...
    try {
      const payload = {
        ...
        email: data.email,
        phone: data.phone,
        eventCity: data.eventCity || exhibitionLocation,
        requirement: `Stall Fabrication for ${exhibitionTitle}`,
        message: data.message
          ? `Stall Dimension: ${data.stallRequirement || "Not specified"}. Message: ${data.message}`
          : `Stall Dimension: ${data.stallRequirement || "Not specified"}. Looking for custom fabrication at ${exhibitionVenue}.`,
      };
      ...
      {status === "success" ? (
        <div className="py-8 text-center flex flex-col items-center gap-4">
          ...
          <p className="text-xs text-white/70 max-w-sm leading-relaxed">
            Our design experts are active for <strong className="text-white">{exhibitionVenue}</strong>. We will get back to you with bespoke 3D stall renders within 24 hours.
          </p>
          <button
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 relative z-10">
          <div>
            ...
            <h3 className="text-lg font-extrabold text-white leading-tight">
              Exhibit at {exhibitionTitle}
            </h3>
            <p className="text-[11px] text-white/60 mt-1 leading-relaxed">
              Partner with Kioskra for turnkey exhibition stall design and build at <strong className="text-white">{exhibitionVenue}</strong>.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              ...
              <option value="12 sqm (3x4m)" className="bg-brand-charcoal text-white">3m × 4m (12 sqm)</option>
              <option value="18 sqm (6x3m)" className="bg-brand-charcoal text-white">6m × 3m (18 sqm)</option>
              <option value="36 sqm (6x6m)" className="bg-brand-charcoal text-white">6m × 6m (36 sqm)</option>
              <option value="Custom Shell Scheme / Bare Space" className="bg-brand-charcoal text-white">Custom / Bare Space (Large)</option>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-bold uppercase tracking-wider text-white/80">
              Additional Brief / Design Guidelines
            </label>
            <textarea
              {...register("message")}
              rows={3}
              placeholder="E.g. budget, brand guidelines, open sides, specific requirements..."
